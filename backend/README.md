...
- Python 3.10+
- PostgreSQL (wrapper'ning o'z collection/agent/tarix ma'lumotlari uchun —
...
curl -s -X POST http://localhost:8100/documents \
...
| Method | Path | Tavsif |
|--------|------|--------|
| POST | `/documents` | `collection` + `files` — hujjat yuklash + parse |
| GET  | `/collections` | collectionlar ro'yxati (tanlash uchun) |
| GET  | `/collections/{collection}/documents` | hujjatlarning parse holati (`status`, `progress`, `all_ready`) — upload'dan keyin shuni pollang |
